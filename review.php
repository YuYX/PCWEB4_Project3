<!-- THERE ARE TWO QUERIES(ONE INSERT AND ONE JOIN) YOU NEED TO FILL FOR THIS FILE. THEY ARE ON LINES 65 AND 109. -->
	...
	<body>
		...
		<?php
			...
			if(isset($_POST["rate"])){
				...
				// Fill in the query to insert the rating into the 
				// database. Use $userid and $movieid for the values of 
				//user_id and movie_id columns respectively
				$query = "INSERT INTO reviews(user_id,movie_id,ratings)
			...
			<div class="row">
				...
				<div class="col-6">
					...
					<?php
						...
						// Fill in the query(JOIN) to join the userinfo and movie_reviews 
						//table together so that you can display the respective rating given 
						//by each username. Use $movieid to filter the records so that only 
						//the ratings for the selected movie is shown.
						
						$query = "SELECT u.username, r.ratings 
						          FROM userinfo u
								  LEFT JOIN reviews r
