<!-- THERE ARE THREE QUERIES(ONE JOIN, ONE DELETE AND ONE SELECT..WHERE..LIKE) YOU NEED TO FILL FOR THIS FILE. THEY ARE ON LINES 75, 99 AND 129. -->
			...
			<div class="row">
				<div class="col-6">
					...
					<div class="searchresult">
						<?php
							// Complete this query to join the userinfo and user_relation tables so 
							// that you can display the usernames of the friends that the current
							// user has. Use $followerid to filter the records so that only those
							// that are friends with the current user is returned.
							$query = "SELECT u.username, re.follower_id, re.following_id
									FROM user_relation re
									LEFT JOIN userinfo u
									ON re.following_id=u.user_id 
									WHERE re.follower_id='$followerid' ";
							$result = mysqli_query($con, $query);
							...
							if(isset($_POST["delrecord"])){
								...
								//Fill in this query to delete the selected friend that the current user 
								//is following(delete from the user_relation table). $followerid is 
								//the id of the current user and $followingid is the id of the 
								//following that you want to delete.	
						...
						<div class="results">
							<?php
								if(isset($_POST["search"])){
									$search = $_POST["user"];
									// Fill in this query to select the usernames which contain the search 
									// term entered by the user. The search term is stored in $search.
