	...
	<body>
		...
		<div class="container allratings">
			<div class="row">
				...
				<div class="col-6">
					<?php
						// Complete the query(JOIN) to join the movies and movie_reviews 
						// table together so that you can display the respective rating 
						//for each movie. Use $userid to filter the records so that only 
						//the ratings given by the selected user is returned.
						$query = "SELECT m.movie_name, r.ratings, r.user_id, r.movie_id
								  FROM movies m 
								  LEFT JOIN reviews r
						...
						if(isset($_POST["delrecord"])){
							...
							// Fill in the query to delete a specific movie review given by 
							// the current user. Use $userid and $movieid to correctly 
							// identify the composite key in the movie_reviews table to 
							// delete the specific movie review.
						...
						if(isset($_POST["rate"])){
							$rating = $_POST["rating"];
							$movieid = $_POST["movieid"];
							// Fill in the query to update a specific movie review given 
							// by the current user. Use $rating as the new value of 
							// rating submitted by the user. Use $userid and $movieid to 
							// correctly identify the composite key in the movie_reviews 
							// table to update the specific movie review.
